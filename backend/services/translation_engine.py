...
from config import GEMINI_API_KEY, OLLAMA_BASE_URL, OLLAMA_MODEL, TRANSLATION_TARGET_LANGUAGE
...
def list_ollama_models(force: bool = False) -> list[str]:
    ...
    try:
        ...
        _OLLAMA_MODELS_CACHE.update({"ts": now, "models": [], "last_error": "Could not reach /api/tags; generation may still work with a selected model"})
        return []
...
def translate_batch(
    ...
    if installed and use_model not in installed:
        raise ValueError(f"Ollama model is not installed: {use_model}")
    # If /api/tags is unavailable or slow, still try /api/generate with the
    # configured/selected model. Remote Ollama servers can be reachable for
    # generation even when discovery timed out.
