"""
Ollama Vision-based manga OCR + Translation service (DEPRECATED).

This module is kept for backward compatibility but is no longer used.
The project now uses MangaOCR for text recognition (runs locally via HuggingFace)
and Ollama/Gemini only for translation.

Legacy behavior: Read text (vision) → Translate → Detect N regions (OpenCV) → Match
"""
import base64
import json
import logging
...
from config import TRANSLATION_TARGET_LANGUAGE, OLLAMA_BASE_URL, OLLAMA_TEXT_MODEL
...
class OllamaOCRService:
	def __init__(self):
		...
		self.model = "llava:7b"  # Legacy fallback (not actively used)
		self.text_model = OLLAMA_TEXT_MODEL
		self._available = None
		self._best_vision_model = None
		logger.warning(f"OllamaOCRService is deprecated. Use MangaOCR for OCR instead.")
	...
	# ── Main pipeline ────────────────────────────────────────────
	def extract_and_translate(self, image_path: str, target_lang: Optional[str] = None) -> Dict:
		...
		try:
			...
			for i, (text, translation) in enumerate(zip(texts, translations)):
				...
				# Convert {x, y, width, height} to [[x1,y1],[x2,y1],[x3,y3],[x4,y4]]
	...
	def translate_text(self, text: str, target_lang: Optional[str] = None) -> Dict:
		if target_lang is None:
			target_lang = TRANSLATION_TARGET_LANGUAGE
